...
class NoticeService implements INoticeService {
    ...
    async uploadAndInsertImage(ref: React.RefObject<RichEditor>): Promise<void | Error> {
        try {
            ...
            ref.current?.insertHTML(
                `<div><br></div><div><img src="${result.uri}" alt="My Image" style="width: 80vw; min-height:80vw; object-fit:cover; border-radius: 10px;" ></div><div><br></div>`
            );
            console.log(ref);
        } catch (err) {
    ...
    async getNotices(buildingId: number): Response<GetNoticesResult> {
        if (buildingId == 0) throw new Error("[getNotices] invalid building id");
